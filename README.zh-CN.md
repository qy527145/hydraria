...
- **认领式多线程抓取** —— `max_threads` 个 worker 各自认领一段连续区间，用一个请求拉回、按绝对偏移落盘。空闲 worker 三级升级要活干：从**最大的空洞**里切（T1）→ 偷别人在途请求的尾巴（T2）→ 只剩尾巴时有预算地对冲（T3）。偷谁看的是慢不是大，30 秒没动静的请求直接判死重切。
...
3. **认领** —— 有效 range `[start, end]` 整段交给调度器；没有预先切好的计划，worker 要一段就现切一段。切多大由调度器按场景决定：下载时是"剩余量 ÷ 空闲线程数"（大分片优先，不做试探），播放时是紧贴读头铺开的等长小分片（默认 2 MiB），只在读者攒出余量后按"余量÷线程数"放大 —— 有序读者只能吐连续前缀，等长分片才能让前缀按聚合速率前进。只有当某一段超时且一个字节都没吐出来（"整段攒齐才发首字节"的中转站）才自动降到 8 MiB 并记住上界。`max_split` 非零时是所有分片的硬上限。
4. **拉取** —— 每个 worker 一个请求一段，按绝对偏移写进本地文件。共享的段尾水位是唯一的抢占机制：别人把它内移，当前 worker 在下一个数据块就干净收工，一个字节都不会重复下载。
5. **拼接** —— 顺序读取端从本地文件读，读到哪里就把临界窗口挪到哪里。落盘顺序无关紧要，只有读取端在乎顺序。
