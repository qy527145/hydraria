...
3. **认领** —— 有效 range `[start, end]` 整段交给调度器；没有预先切好的计划，worker 要一段就现切一段。切多大由调度器按场景决定：下载时是"剩余量 ÷ 空闲线程数"（大分片优先，不做试探），播放时按离读头的距离从 2 MiB 起逐级翻倍。只有当某一段超时且一个字节都没吐出来（"整段攒齐才发首字节"的中转站）才自动降到 8 MiB 并记住上界。`max_split` 非零时是所有分片的硬上限。
