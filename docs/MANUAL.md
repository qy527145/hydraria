...
### 🎚️ 分片大小自适应（`max_split` 默认留空）
"多大的请求最划算"是**源站的性质，不是配置项**：普通 CDN 越大越省 overhead，而某些中转站要把整个 Range 攒齐才吐第一个字节 —— 对它们来说大请求就是永远不返回的请求。

所以默认不填：调度器像 TCP 慢启动一样自己探。从 8 MiB 起步，**成功翻倍、失败减半**，上限 64 MiB，下限 1 MiB。普通源站几个请求内就收敛到"按 worker 数均分剩余工作量"，中转站则停在还能跑通的尺寸下方。

只有源站对单次 Range 长度有硬性要求时才手填，写法是 `5M` / `512K` / `1G`，最小 64K。
...
| 分片大小 | `max_split`，**留空 = 自适应（推荐）**；手填时最小 64K。 |
...
```jsonc
{
  ...
  "max_split": 0,                      // 0 = 自适应(默认); 或 "5M" 这样的可读字符串
...
字段细节：
- `max_split`：0 或留空 = 自适应（8 MiB 起步，成功翻倍、失败减半，1 MiB ~ 64 MiB）。手填时 ≥ 64K，只在源站对单次 Range 长度有硬性要求时才需要。
