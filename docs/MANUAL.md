...
- **播放（延迟优先）** —— 按**离读头的距离**递增。读头正前方那一片只有 2 MiB（用户正在等它），往后 2 / 4 / 8 / 16 / 32 MiB 逐级翻倍：读者在够到某一片之前还要先看完前面的字节，那段"余量"有多长，这一片就可以开多长。seek 之后线程会全部挤到 seek 点附近拉小片，起播快；余量攒起来之后分片自动放大，持续速率不掉。
...
字段细节：
- `max_split`：0 或留空 = 自动（下载按线程均分剩余量；播放按离读头的距离从 2 MiB 起逐级翻倍。只在源站"整段攒齐才发第一个字节"时才自动降到 8 MiB，下限 1 MiB）。手填时 ≥ 64K，是所有分片的硬上限，只在源站对单次 Range 长度有硬性要求时才需要。
